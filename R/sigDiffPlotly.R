...
dat %>% plot_ly(x = x, y = y, mode = "markers", marker = list(size = 12), color = x, colors = "Set1", hoverinfo = "text", text = paste("Read count = ", format(round(dat$y, 2), nsmall = 2))) %>% layout(title = paste("Transcript =", rownames(gene), "<br> FDR =", formatC(permList[[1]][i,]$FDR, format = "e", digits = 2)), xaxis = ax, yaxis = ay, legend = list(x = 0.35, y = -0.26)) %>% add_trace(x = c("DU", "DR"), y= c(mean(dat$y[7:12]),mean(dat$y[1:6])), mode = "lines", showlegend = FALSE, hoverinfo = "text", text = paste(c("(DU)", "(DR)"), "Mean read count =", format(round(c(mean(dat$y[7:12]),mean(dat$y[1:6])), 2), nsmall = 2)))
