...
server <- function(input, output) {
  ...
  p <- ggplot(mtcars, aes(x = wt, y = mpg)) + geom_point(alpha=0) + xlim(0,7) +ylim(-3,3)
  gp <- ggplotly(p)

  set.seed(1)
  pcpDat <- data.frame(ID = paste0("ID",1:2), A=rnorm(2), B=rnorm(2), C=rnorm(2), D=rnorm(2), E=rnorm(2), F=rnorm(2), G=rnorm(2), H=rnorm(2))
...
for (a=0; a<dLength; a++){
  ...
  if (xMin<0){
    var yMinDF1 = dat[cNames[0]]
  }
  if (xMax>(vLength-1)){
    var yMinDC2 = dat[cNames[vLength-1]]
  }
  ...
  // Check triangle cases
  else if (yMinDF1 < yMin && yMinDC2 < yMin && case1 < yMax && yMinDF1 < yMinDC1 && yMinDC1 > yMin){
    //if (xMax<(vLength-1)&&xMin>0){
      console.log(yMinDF1)
      console.log(yMin)
      console.log(yMinDC2)
      console.log(yMinDC1)
  ...
  else if (yMinDF1 > yMax && yMinDC2 > yMax && case1 > yMin && yMinDF1 > yMinDC1){
    if (!isNaN(case1)&&!isNaN(case2)){
