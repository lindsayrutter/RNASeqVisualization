...
libSizes <- as.vector(colSums(d2[[1]]))
libSizes

# Keep gene names
rownames(d2) <- countTable[,1]

# 35852 - In edgeR, it is recommended to remove features without at least 1 read per million in n of the samples, where n is the size of the smallest group of replicates. Also, should filter on CPM!
keep <- rowSums(cpm(d2) > 0.33) >= 4
...
# 35852 - In edgeR, it is recommended to remove features without at least 1 read per million in n of the samples, where n is the size of the smallest group of replicates. Also, should filter on CPM!
# Here, a gene is only retained if it is expressed as a count-per-million (CPM) above 0.5 in at least two samples. keep <- rowSums(cpm(y) > 0.5) >= 2
keep <- rowSums(cpm(d2) > 0.33) >= 4
d2 <- d2[keep, , keep.lib.sizes=FALSE]
...
# filt (15686, 18) These are genes filtered because in bottom quartile of means and stdevs
...
# filt (27387, 16) - Now inclused genes fitlered because in bottom quartile of means and stdevs, and negative residuals
...
# Get 8465 genes and return their raw values (right now they are already logged and normalized with betweenLaneNormalization which cannot be used for DEG)
...
<<eval=FALSE>>=
scatmat(d3, columns=1:16, alpha = 0.01)
@

<<>>=
d3 <- calcNormFactors(d3, method="none")
cpm.d3 <- cpm(d3, TRUE, TRUE)
@

\begin{figure}[H]
\centering
<<fig=TRUE, echo=TRUE>>=
ggparcoord(data.frame(cpm.d3), columns=1:16, alphaLines=0, boxplot=TRUE, scale="globalminmax") + coord_flip()
@
\caption{Boxplot of all samples. F1. and U.2 have some large outliers.}
\label{BoxAll}
\end{figure}
...
\centering
<<fig=TRUE, echo=TRUE>>=
plotMeanVar(d3, show.tagwise.vars=TRUE, NBline=TRUE)
@
\caption{MDS plot.}
...
<<>>=
# Test for differential expression U vs. R
de = exactTest(d3, pair=c("U","R"))
tt = topTags(de, n=nrow(d3))
nc = cpm(d3, normalized.lib.sizes=TRUE)
rn = rownames(tt$table)
@
...
<<fig=TRUE, echo=TRUE>>=
deg = rn[tt$table$FDR < .05]
plotSmear(d3, de.tags=deg)
@
\caption{Create a graphical summary, such as an M (log-fold change) versus A (log-average expression) plot, here showing the genes selected as differentially expressed with a 5\% false discovery rate. There were 56 in this dataset of U versus R.}
\label{plotSmear}
\end{figure}
...
plotSmear(d3, de.tags=deg)
@
\caption{Create a graphical summary, such as an M (log-fold change) versus A (log-average expression) plot, here showing the genes selected as differentially expressed with a 5\% false discovery rate. There were 72 in this dataset of W versus F.}
\label{plotSmear}
\end{figure}
