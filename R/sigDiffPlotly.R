...
plot_ly(data = dat, x = x, y = y, mode = "markers", marker = list(size = 12), color = x, colors = "Set1") %>% layout(title = paste("Transcript:", rownames(gene), " FDR: ", formatC(permList[[1]][i,]$FDR, format = "e", digits = 2)), xaxis = ax, yaxis = ay, legend = list(x = 0.35, y = -.28)) %>% add_trace(x = c("DU", "DR"), y= c(mean(dat$y[7:12]),mean(dat$y[1:6])), mode = "lines", showlegend = FALSE)
